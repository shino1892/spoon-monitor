...
export async function initSpoon(type: "DJ" | "MONITOR") {
    ...
    const accessToken = type === "DJ" ? process.env.DJ_ACCESS_TOKEN : process.env.MONITOR_ACCESS_TOKEN;
    const refreshToken = type === "DJ" ? process.env.DJ_REFRESH_TOKEN : process.env.MONITOR_REFRESH_TOKEN;

    if (!accessToken || !refreshToken) throw new Error(`${type} tokens are missing.`);
